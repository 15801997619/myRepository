...
<rapid:override name="content">
	...
	<div class="queryPanel">
		...
		<div class="handleControl">
			<input type="submit" class="form-btn" style="width:80px" value="查询" onclick="getReferenceForm(this).action='<@jspEl 'ctx'/>${actionBasePath}/list.do'"/>
			<input type="button" class="form-btn" style="width:80px" value="新增" onclick="window.location='<@jspEl 'ctx'/>${actionBasePath}/create.do'"/>
			<input type="button" class="form-btn" style="width:80px" value="删除" onclick="batchDelete('<@jspEl 'ctx'/>${actionBasePath}/delete.do','items',document.forms.queryForm)"/>
		</div>
	</div>
		...
		<table width="100%"  border="0" cellspacing="0" class="gridBody">
		  ...
		  <tbody>
			  ...
			  <tr class="<@jspEl "status.count % 2 == 0 ? 'odd' : 'even'"/>">
				...
				<#if column.pk>
				<td>
					<a href="<@jspEl 'ctx'/>${actionBasePath}/show.do?${column.columnNameLower}=<@jspEl "item['${column.sqlName}']"/>">查看</a>&nbsp;&nbsp;&nbsp;
					<a href="<@jspEl 'ctx'/>${actionBasePath}/edit.do?${column.columnNameLower}=<@jspEl "item['${column.sqlName}']"/>">修改</a>
				</td>
				</#if>
				</#list>
			  </tr>
